...
public class NetworkedCacheableImageView extends CacheableImageView {
	...
	private static class ImageUrlAsyncTask extends
		...
		ImageUrlAsyncTask(ImageView imageView, BitmapLruCache cache,
				...
				OnImageLoadedListener listener, boolean IsLocal) {
			mCache = cache;
		...
		protected CacheableBitmapDrawable doInBackground(String... params) {
			try {
				...
				if (null == result) {
					...
					if (!mIsLocal) {
						LoggerTool
								.d("ImageUrlAsyncTask", "Downloading: " + url);
						// 从网络下载图片
	...
	public boolean loadImage(String url, final boolean fullSize,
		...
		if (null != wrapper) {
			...
			if (listener != null) {
				listener.onImageSet((CacheableBitmapDrawable) wrapper);
			}
			return true;
		} else {
	...
	public boolean loadImage(String url, BitmapFactory.Options decodeOpts,
			OnImageLoadedListener listener, boolean IsMediaUrl) {
		// First check whether there's already a task running, if so cancel it
		if (null != mCurrentTask) {
			mCurrentTask.cancel(true);
		}
